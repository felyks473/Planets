...
namespace Planets {
    ...
    void World::Update(std::vector<std::shared_ptr<Shader>> shaders, CameraSystem camera, std::vector<bool*>& stop, std::vector<float*>& slider_value) const
    {
        systemManager->Update(shaders, camera, stop, slider_value);
    }
